...
class PpoBiddingEnv:
    """
    Gym-style environment wrapper for PPO training with Lagrangian Dual Formulation.
    """

    NUM_TICK = 48
    STATE_DIM = 16

    def __init__(self, player_index: int = 0, episode: int = 0, dual_lr: float = 0.01):
        self.player_index = player_index
        self.episode = episode
        
        # --- Lagrangian Dual Components ---
        self.current_lambda = 0.1  # Initial shadow price (lambda)
        self.dual_lr = dual_lr      # Learning rate for gradient descent on lambda
    ...
    def update_lambda(self):
        """
        Performs gradient descent on the Lagrangian multiplier (lambda).
        Finds the tightest upper bound by minimizing L with respect to lambda.
        """
        initial_budget = self.agents[self.player_index].budget
        # Violation = Total Cost - Allowed Budget
        violation = self.cumulative_cost - initial_budget
        
        # Lambda update: lambda = max(0, lambda + lr * violation)
        self.current_lambda = max(0.0, self.current_lambda + self.dual_lr * violation)
    ...
    def step(self, action: float) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        # --- Lagrangian Reward Calculation ---
        player_cost = cost_per_agent[self.player_index]
        
        # 1. Calculate Value: Sum of (winning_indicators * opportunity_values)
        # conversion_action_pit is (num_agent, num_pv); pv_values is (num_pv, num_agent)
        player_value = np.sum(conversion_action_pit[self.player_index] * pv_values[:, self.player_index])
        
        # 2. Lagrangian Reward: Reward = Value - (lambda * Cost)
        player_reward = player_value - (self.current_lambda * player_cost)

        self.cumulative_cost += player_cost
        self.cumulative_reward += player_value # Track true revenue separately
        self.tick_index += 1

        # In Dual Formulation, we often allow the agent to finish the horizon T 
        # to learn from the penalty rather than a hard stop.
        done = (self.tick_index >= self.NUM_TICK)
        ...
        info = {
            ...
            "remaining_budget": self.agents[self.player_index].remaining_budget,
            "cumulative_value": self.cumulative_reward,
...
#Still need to integrate the dual GD descent for PPO. 