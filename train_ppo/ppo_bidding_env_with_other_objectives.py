...
class PpoBiddingEnv:
    ...
    def step(self, action: float) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        # --- Update budgets (version 2),  sum of conversion actions (x_ij) weighted by pValues (v_ij)---
        # Calculate Weighted Reward (sum of x_ij * v_ij)
        
        #Calculate reward as sum of conversion actions (x_ij) weighted by pValues (v_ij)
        #reward_per_agent = (conversion_action_pit * pv_values.T).sum(axis=1)

        # --- Update budgets (version 3), logarithmic penalty for overspend ---
        # Calculate Weighted Reward (sum of x_ij * v_ij)
        
        #Calculate reward as sum of conversion actions (x_ij) weighted by pValues (v_ij)
        #reward_per_agent = (conversion_action_pit * pv_values.T).sum(axis=1)

        # 3. Apply Logarithmic Barrier Function for each agent
        #mu = 0.05  # Stiffness hyperparameter to tune
        #for i, agent in enumerate(self.agents):
            # We use a small epsilon (1e-6) to prevent log(0) if budget is exhausted
            #barrier_term = mu * np.log(max(1e-6, agent.remaining_budget))
            #reward_per_agent[i] += barrier_term
